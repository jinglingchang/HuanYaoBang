    ...
    if (is_array($dirName)) {
        foreach ($dirName as $dirValue) {
            if (!empty($dirValue) && $dirValue != '') {
                ...
                $filePath = WEBSITE_ROOT_DIR . '/' . $fileKey . '/' . $dirValue . '/';
                //echo $filePath.'<br />';
                fileRead($filePath);
            }
        }
    } else {
        $filePath = WEBSITE_ROOT_DIR . '/' . $dirName . '/';
...
function import($file_name){
    ...
    if(!empty($file_name)){
        ...
        if(!empty($file_name_array[0])){
            ...
            switch($file_name_array[0]){

                case 'controller':

                    $path = WEBSITE_ROOT_CRONTROLLER_DIR.'/'.$file_name_array[1].'Controller.php';

                    break;

                case 'model':

                    $path = WEBSITE_ROOT_Model_DIR.'/'.$file_name_array[1].'Model.php';

                    break;

                case 'dal':

                    $path = WEBSITE_ROOT_Dal_DIR.'/'.$file_name_array[1].'Dal.php';

                    break;

                case 'viewModule':

                    $path = WEBSITE_ROOT_ViewModule_DIR.'/'.$file_name_array[1].'ViewModule.php';
